 ...
 * @returns
 */
async function findDependencies(devServer, match) {
	const deps = new Set();
	try {
 ...
 * @returns
 */
async function findStylesInModuleGraph(devServer, match) {
	const styles = {};
	const dependencies = await findDependencies(devServer, match);
